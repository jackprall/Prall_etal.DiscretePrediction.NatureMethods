...
# Create a function to generate a posterior predictive distribution from a
#   Bayesian phylogenetic logistic regression with a single covariate
#   (latent-data formulation). =================================================
## This function only works when predicting one data point.
## Dependency: ape.
post_pred_phy_latent <- function(
  tree_full,
  test_species,
  y_train,
  x_train,
  ...
  C_full <- ape::vcv(phy = tree_full)
  C_ih_temp <- C_full[, test_species]
  C_ih <- as.matrix(C_ih_temp[names(C_ih_temp) != test_species])
  tree_train <- ape::drop.tip(phy = tree_full, tip = test_species)
  C <- ape::vcv(phy = tree_train)
  Y <- as.matrix(y_train)
  X <- as.matrix(cbind(rep(1, length(x_train)), x_train))
  c_hh <- as.numeric(C_ih_temp[names(C_ih_temp) == test_species])
  c_h <- c_hh - t(C_ih) %*% solve(C) %*% C_ih
  ppd_latent <- NULL
  for (i in 1:nrow(mcmc_samp)) {
    Beta <- as.matrix(c(mcmc_samp[[i, 1]], mcmc_samp[[i, 2]]))
    if (variance == TRUE) {
      sigma2_species <- mcmc_samp[[i, 3]]
    } else {
      sigma2_species <- (mcmc_samp[[i, 3]])^2
    }
    s_mu <- t(C_ih) %*% solve(C) %*% (Y - (X %*% Beta))
    s_sig2 <- sigma2_species * c_h
    s <- rnorm(n = 1, mean = s_mu, sd = sqrt(s_sig2))
    e <- rnorm(n = 1, mean = 0, sd = 1.6)
    ppd_latent[i] <- Beta[1, 1] + Beta[2, 1] * x_test + s + e
  }
  ppd_pi <- pnorm(
    q = 0,
    mean = ppd_latent,
  ...
  ppd_y_new <- as.numeric(ppd_latent > 0)
  output <- list(ppd_latent, ppd_pi, ppd_y_new)
  names(output) <- c("latent", "pi", "y_new")
  return(output)
}
...
post_pred_phy_logit <- function(
  ...
  for (i in 1:nrow(mcmc_samp)) {
    ...
    s_sig2 <- sigma2_species * c_h
    s <- rnorm(n = 1, mean = s_mu, sd = sqrt(s_sig2))
    ppd_pi[i] <- plogis(Beta[1, 1] + Beta[2, 1] * x_test + s)
  }
  ppd_y_new <- rbinom(n = length(ppd_pi), size = 1, prob = ppd_pi)
  output <- list(ppd_pi, ppd_y_new)
  names(output) <- c("pi", "y_new")
  return(output)
}
