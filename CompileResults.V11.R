...
for (type in types) {
  ...
  Results.Full <- matrix(nrow = 0, ncol = 39)
  colnames(Results.Full) <- c("Trial_#", "Taxon_#", "Trait_A", "Trait_B", "Terminal_Branch_Length",
                              "Beta_Bin_Prob", "Naive_Prob", "NP_LR_Prob", "Phy_LR_Prob", "MS_MCMC_Prob", "Ind_MCMC_Prob", 
                              "Dep_MCMC_Prob", "MS_RJ_Prob", "Ind_RJ_Prob", "Dep_RJ_Prob",
                              "Beta_Bin_Acc", "Naive_Acc", "NP_LR_Prob", "Phy_LR_Prob", "MS_MCMC_Acc", "Ind_MCMC_Acc",
                              "Dep_MCMC_Acc", "MS_RJ_Acc",  "Ind_RJ_Acc",  "Dep_RJ_Acc",
                              "Beta_Bin_LL", "Naive_LL", "NP_LR_Prob", "Phy_LR_Prob", "MCMC_MS_LL", "MCMC_Ind_LL", 
  ...
  for (i in 1:trial_amount) {
    ...
    # Redundancy helps cover situations where there may be multiple predictions
    NB.Predictions <- Naive_Bayes_predict(counts, rTaxonValueA)

    # Finally, we will pull the results of the logistric regression tests
    if (length(rTaxon) > 1) {
      Bname <- paste0("./Results/", type, "TraitBInfo.txt")
      Bdata <- read.table(file = Bname, row.names = NULL)
      Bdata <- Bdata[,-1]
      ...
      NPGlmmName <- paste0("./Results/", type, ".NP.GLMM.Results.txt")
      PhyGlmmName <- paste0("./Results/", type, ".PH.GLMM.Results.txt")

      NPGlmmData <- read.table(file = NPGlmmName, skip = 1)
      PhyGlmmData <- read.table(file = PhyGlmmName, skip = 1)
    }
    ...
    for (j in 1:length(rTaxon)) {
      ...
      if (length(rTaxon) > 1) {
        ...
        NPGlmm.Prob <- NPGlmmData[i, 2]
        PhyGlmm.Prob - PhyGlmmData[i, 2]
      }
      ...
      if (length(rTaxon) > 1) {
        ...
        NPGlmm.acc <- NPGlmmData[i, 3]
        PhyGlmm.acc - PhyGlmmData[i, 3]
      }
      ...
      if (length(rTaxon) > 1) {
        ...
        NPGlmm.LL <- NPGlmmData[i, 4]
        PhyGlmm.LL - PhyGlmmData[i, 4]
      }
      ...
      Trial <- c(i, rTaxon[j], rTaxonValueA[j], rTaxonValueB[j], TBL,
          BB.Prob, NB.Prob, NPGlmm.Prob, PhyGLMM.Prob, MS.MCMC.Prob, Ind.MCMC.Prob,
          Dep.MCMC.Prob, MS.RJ.Prob, Ind.RJ.Prob, Dep.RJ.Prob,
          BB.acc, NB.acc, NPGlmm.acc, PhyGLMM.acc, MS.MCMC.acc, Ind.MCMC.acc,  
          Dep.MCMC.acc, MS.RJ.acc,  Ind.RJ.acc,  Dep.RJ.acc,
          BB.LL, NB.LL,  NPGlmm.LL, PhyGLMM.LL, MS.MCMC.LL, Ind.MCMC.LL, 
